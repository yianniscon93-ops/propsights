"""
Bazaraki.com — for-sale property listings scraper.

Pipeline:
  search_sale_listings() → bazaraki_sale_log (append) → bazaraki_sale_latest (upsert)
...
from scraper.bazaraki_client import search_sale_listings
...
def run(max_pages: int | None = None) -> None:
    ...
    log.info(f"Bazaraki sale scraper started — {execution_timestamp:%Y-%m-%d %H:%M:%S}")
    _notify(f"Bazaraki sale run started — {execution_timestamp:%Y-%m-%d %H:%M}")

    conn = init_db(DB_PATH)
    try:
        init_bazaraki_tables(conn)

        listings = search_sale_listings(proxy_url=PROXY_URL, delay=DELAY, max_pages=max_pages)

        written = write_bazaraki_log(conn, listings, "sale", execution_timestamp)
        summary = update_bazaraki_latest(conn, "sale", execution_timestamp)
        log.info(
        ...
        log.exception("Bazaraki sale run failed")
        _notify(f"Bazaraki sale run FAILED: {e}")
        raise
    finally:
        conn.close()
    ...
    _notify(
        f"Bazaraki sale done ✓ — {elapsed // 60}m {elapsed % 60}s  "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-pages", type=int, default=None, help="Limit pages per rubric (for testing)")
