...
/* ── Neighbourhood (hyper-local) helpers ─────────────────────── */
...
/* ── Draw overlay (animated "lasso" around a neighbourhood) ───── */
  ...
  return (
    ...
    >
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
        {/* Spotlight: dim everything except the drawn area */}
        <motion.path
          d={DIM_PATH}
          fillRule="evenodd"
          fill="#0C100A"
        ...
        />
        {/* Subtle green fill inside the area */}
        <motion.path
          d={HOOD_PATH}
          fill="#8FCC80"
          stroke="none"
...
/* ── Sample listings (representative, not a live feed) ────────── */
...
/* ── Dashboard preview (scoped to the drawn neighbourhood) ────── */
...
function DashboardView({ area }: { area: string }) {
  ...
  // Seasonality curve (occupancy by month) as an area chart.
  ...
  // Neighbourhood vs city comparison bars.
...
function caption(stage: Stage, area: string) {
  switch (stage) {
    case "map":
      return "Scanning all of Cyprus";
    case "zoom":
      return `Zooming into ${area}`;
    case "draw":
      return `Outlining ${hoodName(area)}`;
    case "listings":
      return `${hoodListingCount(area)} listings in ${hoodName(area)}`;
    case "dashboard":
      return `${hoodName(area)} insights`;
  }
}
    ...
    <div className="w-full">
      ...
      >
        ...
        <div className="relative" style={{ height: "clamp(440px, 64vh, 640px)", background: "#E8EDE3" }}>
          {/* Map layer (always mounted). `isolation: isolate` contains Leaflet's
              internal pane z-indexes so the overlays/caption render above the map. */}
          <div
          ...
          {/* Caption chip — only over the map stages (listings/dashboard have
              their own headers, so the chip would overlap their content). */}
          {mapVisible && (
